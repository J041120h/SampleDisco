...
cluster-emb obsm key (and optionally a sample-preserved emb for CMD) and
...
Algorithm (matches the singleCMD recipe; no CLR by default):
    ...
    4. CMD — per-(group, cluster) LOO displacement from the cmd_emb;
...
def _resolve_cmd_emb_key(adata, cluster_emb_key: str,
...
def compute_sample_embedding(
    ...
    *,
    ...
    """Compute sample-level embedding (singleCMD recipe).
    ...
    # ---- CMD: per-(group, coarse cluster) LOO displacement ----
    if use_cmd:
        if verbose:
            print(f"[CMD] LOO displacement on cmd_emb...", flush=True)
        # Build per-unit cells from the cmd_emb (might differ from cluster_emb)
