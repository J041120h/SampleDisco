...
def add_parent_references(tree):
    """Add parent pointers to all clades (Bio.Phylo omits these by default)."""
...
def calculate_distance_matrix_from_tree(tree):
    """Compute pairwise patristic distance matrix from a Bio.Phylo tree."""
    ...
    def get_distance_to_mrca(terminal, mrca):
        """Sum branch lengths from terminal up to mrca."""
        distance = 0
        current = terminal
