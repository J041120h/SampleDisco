...
def best_alpha(path):
    txt = open(path).read()
    m = re.search(r"best rmd_weight\s*:\s*([\d.]+)", txt)
...
for axi, (cols, label) in enumerate((
    ...
    ax[axi].set_xlabel("rmd_weight α (log)")
...
fig.suptitle("Mode B (diemb, Z_clust + Z_rmd) — per-modality CCA across α, "
...
lines.append("-" * 78)
lines.append("  scGLUE run 1  : use_batch=batch     → obsm['Z_rmd']    (sample-preserved)")
...
lines.append("  SE            : composition (A1+A2+A3) + RMD; α (rmd_weight) auto-tuned")
...
cmp_row("rmd_weight α",                           "rmd_weight")
...
if r_modeA is not None:
    lines.append(f"    rmd_weight α                       : {r_modeA['rmd_weight']:.4f}")
...
lines.append("     blocks get more relative weight; RMD less. The autotune is willing")
lines.append("     to give up some RMD weight because RNA's bio signal is already")
...
lines.append("     (Z_clust + Z_rmd) carries sev.level signal coherently across both")
